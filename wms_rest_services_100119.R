...
apiKey <- "Ar2zhWwAE7IwTl1fYHkk5CK9WwjghmlHFz67Dk8kS_f0U_0bcP8z7qFGh52jN-dh"
...
leaflet() %>% setView(-71, -2.5, 7) %>% 
  ...
  addEsriImageMapLayer(
    url = paste0("http://gis-treecover.wri.org/arcgis/rest/services/ForestCover_last/ImageServer"),
    group = "Landsat 2012 (Hansen/UMD)",
    options = imageMapLayerOptions(bandIds = (('Band_2,Band_3,Band_1'))),
    #options = WMSTileOptions(format = 'image/png', transparent = TRUE)
  ) %>%

  addEsriFeatureLayer(
    url = paste0("https://geo.socioambiental.org/webadaptor2/rest/services/raisg/raisg_tis/MapServer/3"),
    group = "Indigenous Territory (RAISG)",
...
# GLOBAL FOREST CHANGE - FOREST LOSS DENSITY
  addEsriImageMapLayer(
    url = "http://gis-treecover.wri.org/arcgis/rest/services/ForestCover_lossyear_density/ImageServer",
    #layers = 'ForestCover_lossyear',
    group = "Forest Cover Loss (Hansen/UMD)",
    options = imageMapLayerOptions(opacity = 0.75, position = "back", noData = 0),
    #options = WMSTileOptions(format = 'image/png', transparent = TRUE)
  ) %>%
  ...
  hideGroup("Forest Cover Loss (Hansen/UMD)") %>%
  hideGroup("Roads (RAISG)") %>%
  hideGroup("Protected Areas (RAISG)") %>%
  hideGroup("Indigenous Territory (RAISG)") %>%
  ...
  addLayersControl(
    overlayGroups = c("Forest Cover Loss (Hansen/UMD)", "Protected Areas (RAISG)", "Indigenous Territory (RAISG)", "Mining (RAISG)", "Petroleum Blocks (RAISG)", "Roads (RAISG)"), #"Mammals (GBIF)", "Amphibians (GBIF)"),
    baseGroups = c("Canvas Dark", "Landsat 2012 (Hansen/UMD)", "Bing Imagery"), #, "Population Density (SEDAC)"),#, "Development Threat (SEDAC)"),
